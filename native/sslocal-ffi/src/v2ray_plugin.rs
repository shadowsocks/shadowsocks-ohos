...
// Vendored from meow-rs (GPL-3.0): crates/meow-proxy/src/v2ray_plugin.rs @ v0.18.0
// Adapted: `meow_common` errors replaced by `crate::plugin::PluginError`,
// `tracing` replaced by `log`, and `dial` connects TCP itself with tokio.
...
//!   and returns a `Box<dyn meow_transport::Stream>` ready to relay
//!   Shadowsocks traffic over.
...
pub fn parse_opts(s: &str) -> Result<V2rayPluginConfig, PluginError> {
        ...
        match key {
            "mode" => {
                if value.eq_ignore_ascii_case("websocket") || value.eq_ignore_ascii_case("ws") {
                    ...
                    return Err(PluginError::InvalidOptions(format!(
...
/// and return the framed stream ready to carry Shadowsocks traffic.
pub async fn dial(
    ...
    // 1) Raw TCP.
    let tcp = tokio::net::TcpStream::connect((server_host, server_port)).await?;
    let stream: Box<dyn meow_transport::Stream> = Box::new(tcp);

    // 2) Optional TLS handshake. The layer is built per connection because
    // the SNI fallback depends on the server host.
    let stream = if cfg.tls {
        let tls_config = TlsConfig {
            skip_cert_verify: cfg.skip_cert_verify,
            ..TlsConfig::new(host_header.clone())
        };
        TlsLayer::new(&tls_config)?.connect(stream).await?
    } else {
        stream
    };
    ...
    Ok(ws_layer.connect(stream).await?)
}
