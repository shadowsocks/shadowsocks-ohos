...
// Vendored from meow-rs (GPL-3.0): crates/meow-proxy/src/simple_obfs.rs @ v0.18.0
...
//!
//! Used as the transport wrapper for SIP003 `obfs-local` / `simple-obfs`
//! plugin configs (`obfs=http|tls;obfs-host=...`). No external `obfs-local` /
//! `simple-obfs` binary is required — essential on HarmonyOS, where spawning
//! plugin processes is impossible.
        ...
        if this.first_response {
            ...
            loop {
                ...
                if let Some(idx) = find_double_crlf(&this.response_scratch) {
                    ...
                    // Deviation from the upstream vendored source: when the
                    // fake response headers arrive without any body byte in
                    // the same read, fall through to the passthrough read
                    // instead of returning a 0-byte read — callers such as
                    // tokio's copy_bidirectional treat that as EOF.
